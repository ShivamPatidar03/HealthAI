...
const cardVariants = cva("absolute will-change-transform", {
  variants: {
    variant: {
      dark: "flex size-full flex-col items-center justify-center gap-6 rounded-[2rem] border border-stone-700/50 bg-[#0f172a]/80 p-6 backdrop-blur-xl shadow-2xl",
      light:
        "flex size-full flex-col items-center justify-center gap-6 rounded-[2rem] border bg-accent bg-background/80 p-6 backdrop-blur-xl shadow-2xl",
    },
  },
  defaultVariants: {
    variant: "light",
...
export const ContainerScroll = ({ children, style, className, ...props }) => {
  ...
  // NOTE: Depending on Framer Motion version, standard setup with `offset` works.
  ...
  return (
    <ContainerScrollContext.Provider value={{ scrollYProgress }}>
      <div
        ref={scrollRef}
        className={cn("relative min-h-[150vh] w-full", className)}
    ...
    <div className={cn("flex items-center gap-2", className)} ref={ref} {...props}>
      <div className="flex items-center">
        ...
        {[...Array(emptyStars)].map((_, index) => (
          <svg key={`empty-${index}`} className="size-4 text-gray-400" fill="currentColor" viewBox="0 0 20 20">
            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.286 3.957a1 1 0 00.95.69h4.162c.969 0 1.371 1.24.588 1.81l-3.37 2.448a1 1 0 00-.364 1.118l1.287 3.957c.3.921-.755 1.688-1.54 1.118l-3.37-2.448a1 1 0 00-1.175 0l-3.37 2.448c-.784.57-1.838-.197-1.54-1.118l1.287-3.957a1 1 0 00-.364-1.118L2.05 9.384c-.783-.57-.38-1.81.588-1.81h4.162a1 1 0 00.95-.69l1.286-3.957z" />
          </svg>
        ))}
      </div>
